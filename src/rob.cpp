...
void RoB::work() {
  ...
  if (rob_get_in && rob_error != 1) {
    ...
    ready[ii] = to_unsigned(from_rs_wire_ready);
    dest[ii] = to_signed(from_rs_wire_dest);
  }
  if (lsb_get_in && rob_error != 1) {
    int ii = to_unsigned(from_lsb_dest) % ROB_SIZE;
    ready[ii] = 1;
    a[ii] = to_signed(from_lsb_wire_value);
  }
}