  ...
  void work() override {
    if (issue) {
      ...
      case SRL:
        ...
        break;
      case SRA:
        out <= (to_signed(rs1) >> to_unsigned(rs2));
      case AND:
        out <= (rs1 & rs2);
        break;
      case OR:
      ...
      case SNEQ:
        ...
        break;
      default:
        dark::debug::assert(false, "Invalid opcode");
      }
      done <= 1;
    } else {
      done <= 0;
...
int main() {
  ...
  std::unordered_map<std::string, Opcode> cmd2op = {
      {"add", Opcode::ADD},   {"sub", Opcode::SUB},  {"sll", Opcode::SLL},
      {"src", Opcode::SRL},   {"sra", Opcode::SRA},  {"and", Opcode::AND},
      {"or", Opcode::OR},     {"xor", Opcode::XOR},  {"slt", Opcode::SLT},
      {"sltu", Opcode::SLTU}, {"sge", Opcode::SGE},  {"sgeu", Opcode::SGEU},
      {"seq", Opcode::SEQ},   {"sneq", Opcode::SNEQ}};
