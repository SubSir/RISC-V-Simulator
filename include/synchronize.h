...
struct Visitor {
  ...
  static constexpr bool is_syncable_v = requires(_Tp &val) {
    { val.sync() } -> std::same_as<void>;
  };

  template <typename _Tp>
    requires is_syncable_v<_Tp>
  static void sync(_Tp &val) {
    val.sync();
  }

  template <typename _Tp, typename _Base> static _Base &cast(_Tp &value) {
    return static_cast<_Base &>(value);
  }
};

template <typename... _Base> struct SyncTags {};

template <typename _Tp> static constexpr bool is_valid_tag_v = false;
...
template <typename _Tp> inline void sync_member(_Tp &value);
